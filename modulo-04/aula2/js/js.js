...
console.log('series antes depois de 2017',filtrarSeriesPorAno(series, 2017))
...
function queroGenero(series, gene){
  var retorno = []
  series.forEach(function(serie){
    for(gen of serie.genero) {
      if(gen === gene) {
        //console.log(gene)
        retorno.push(serie)
      }
...
console.log('queroGenero Caos:',queroGenero(series, "Caos"))

function queroTitulo(series, parte){
  var retorno = []
  series.forEach(function(serie){
      if(serie.titulo.indexOf(parte) !== -1) {
        //console.log(gene)
        retorno.push(serie)
    }
  })
...
console.log('queroTitulo The:',queroTitulo(series, "The"))

/*
Exercicio 7: Creditos
Ao final de um episódio, temos os créditos do episódio. Para isso vamos implementar uma função,
 chamada de creditosIlluminatis Recebendo uma serie como parâmetro e imprima os créditos a partir dela.
  Deverá ser impresso, o Titulo da serie, os Diretores, avisando com um título que é o bloco deles;
  Em seguida vem o elenco, também com um título de Elenco;
Tranquilo né? Easy! MAS, tem o seguinte: Os créditos são sempre ordenados alfabéticamente,
 mas pelo ULTIMO NOME!! Faça os ajustes necessários para que isso seja possível
...
function creditosIlluminatis(serie){
  var stringao = ""
  stringao = stringao.concat(serie.titulo, "\nDiretores\n")
  var arrayDiretor =[]
  for(diretores of serie.diretor) {
    arrayDiretor.push(diretores)
  }
  ordenarDecrescente(arrayDiretor);
  for(diretores of arrayDiretor) {
    stringao = stringao.concat(diretores, ", ")
  }
  stringao = stringao.concat('\nElenco\n')
  var arrayAtor = []
  for(atores of serie.elenco) {
    arrayAtor.push(atores)
  }
  ordenarDecrescente(arrayAtor)
  for(atores of arrayAtor) {
    stringao = stringao.concat(atores, ", ")
  }
  return stringao
}
...
function todosAbreviados(series) {
  var detectados = 0
  var elencoEspecial
  series.forEach(function(serie){
    for(ator of serie.elenco) {
      if(detectaAbreviacoes(ator)){
        detectados++
        if(serie.elenco.length === detectados) {
          elencoEspecial = serie.elenco
          return
        }
      }
    }
  ...
  return elencoEspecial
}
function detectaAbreviacoes(ator) {
  if(ator.indexOf('.') !== -1) {
    return true
  }
  return false
...
var elencoEA = todosAbreviados(series)

function easterEgg(elenco) {
  var stringao = ""
  var stringao2 = ""
  for(nome of elenco) {
      stringao2 = stringao2.concat(nome,'')
  }
  arrayZao = stringao2.split(' ')
  for(var i = 0; i < arrayZao.length; i++) {
    if(arrayZao[i].indexOf('.') === -1){
      arrayZao.splice(i, 1)
      i = i-1
      stringao = stringao.concat(arrayZao[i+1])
    }
  }
  stringao = arrayZao.join('')
  arrayZao = stringao.split('.')
  stringao = '#'
  stringao = stringao.concat(arrayZao.join(''))
  return stringao
}

console.log(easterEggIlluminati = easterEgg(todosAbreviados(series)))
